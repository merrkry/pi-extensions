...
`dispose` (10 lines, cognitive ~2) and `update`'s idle path share _some_ statements — `clearInterval`, `setWidget(undefined)`, `setStatus(undefined)`, flag resets — but differ in two ways:
