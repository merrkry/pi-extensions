...
- Key design decision beyond the issue sketch: `clear()` must _settle_ dropped pending promises (resolve them), not just drop the thunks.
...
- Verified no production caller awaits a _queued_ agent's promise in a blocking way (`get-result-tool.ts` guards on `status === "running"`; `spawnAndWait` is foreground/direct; `waitForAll` filters by status), confirming it is safe to give queued agents a real promise.
